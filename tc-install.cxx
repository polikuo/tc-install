...
void selectFile() {
  ...
  Fl_File_Chooser chooser("/",target.c_str(), Fl_File_Chooser::SINGLE, target.c_str());
  chooser.show();
  while(chooser.shown())
    { Fl::wait(); }
...
void brwCB(Fl_Widget*) {
  if ( brwTarget->value() )
  {
    ...
    // iss >> partitionNbr;
    // sdXY, X: [a-z]; Y: [1-15];
    // mmcblkZpW, Z: [0-9]; Y: [1-15];
    if (dev.length() == 4) {
      partitionNbr = (dev.c_str())[3] - 48;
    }
    if (dev.length() == 5) {
      partitionNbr = (dev.c_str())[4] - 48 + 10;
    }
    if (dev.length() == 9) {
      partitionNbr = (dev.c_str())[8] - 48;
    }
    if (dev.length() == 10) {
      partitionNbr = (dev.c_str())[9] - 48 + 10;
    }
    grpButtons->activate();
